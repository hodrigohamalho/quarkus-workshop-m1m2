...
=== Customizing the Developer Perspective in OpenShift

To better observe what’s happening in your OpenShift project, let's customize your side menu by adding a few useful shortcuts.

* Click on `Search` in the left-hand side menu.
* Search for `Kafka`.
* Click on *Add to Navigation*.
* Repeat the same process for the following resources: `KafkaTopic` and `Pod`.

image::ocp-navigation-01.png[ocp-navigation,800]

After adding all the items, your navigation menu should now look like this:

image::ocp-navigation-02.png[ocp-navigation2,800]
...
These are all of the Kafka cluster elements you can install. Select **Create**. This opens a YAML editor allowing you to configure the cluster before deploying it. Change the name of the cluster from `my-cluster` to `names-cluster` (under the _metadata_ section of the YAML file). Leave all other values as-is, and select **Create**:
...
To see the Kafka Cluster being created, just select the `Pods` option that we just added to the Openshift side menu. The object being created are names-cluster-kafka-X and names-cluster-zookeeper-X. 

image::kafka-creation.png[kafkacreation,800]
...
== Create Kafka Console UI

You can choose your preferred Kafka UI to visualize things as read-only. 

In this example we are going to deploy a Kafka UI. 

oc project {{ USER_ID }}-project
oc create -f https://raw.githubusercontent.com/hodrigohamalho/quarkus-workshop-m1m2/refs/heads/ocp-4.15/files/kafka-ui.yml

ALERT: This route isn't HTTPS, use HTTP. 

After that, return to the Topology View and see if you can access the Kafka UI Console. 

image::kafka-ui.png[kafkaui,800]
...
Click on the `Topics` section in the OpenShift side menu and see if the topic named `names` was created. 

image::kafkatopics.png[kafkatopics,800]
...
The process method is invoked for each message from the Kafka topic `names` (configured in the application configuration). Every result is sent to the my-data-stream in-memory stream.
...
%prod.quarkus.openshift.route.tls.insecure-edge-termination-policy=None
...
This guide demonstrated how to build a reactive streaming application using Quarkus and Kafka. It utilizes MicroProfile Reactive Messaging to build data streaming applications.
...
You may have noticed that our application both **produces** and **consumes** events, acting simultaneously as a **Producer** and a **Consumer** in Kafka.

Let's review some important parameters that can help us tune Kafka producers and consumers for better performance and reliability.

=== Producer Parameters

- `acks=all`: Ensures the strongest delivery guarantee (all brokers must acknowledge).
- `retries`: Number of retry attempts on failure.
- `compression.type`: Network optimization using `snappy`, `gzip`, or `lz4`.
- `linger.ms`: Time to wait before sending a batch of messages (improves throughput).
- `batch.size`: Maximum size in bytes of a batch of messages.
- `buffer.memory`: Total memory available to the producer for buffering.

These can be applied in the `application.properties` file as follows:

[source,properties,role="copypaste"]
----
mp.messaging.outgoing.meu-topico-out.connector=smallrye-kafka
mp.messaging.outgoing.meu-topico-out.topic=meu-topico
mp.messaging.outgoing.meu-topico-out.value.serializer=org.apache.kafka.common.serialization.StringSerializer
mp.messaging.outgoing.meu-topico-out.acks=all

mp.messaging.outgoing.meu-topico-out.retries=5
mp.messaging.outgoing.meu-topico-out.compression.type=snappy
mp.messaging.outgoing.meu-topico-out.linger.ms=10
mp.messaging.outgoing.meu-topico-out.batch.size=32768
mp.messaging.outgoing.meu-topico-out.buffer.memory=67108864
----

=== Consumer Parameters

- `auto.offset.reset`: Determines where to start reading when no committed offset is found.
- `enable.auto.commit`: Disables automatic offset commit to allow manual control.
- `fetch.min.bytes` and `fetch.max.wait.ms`: Control how much data is fetched per poll.
- `max.partition.fetch.bytes`: Maximum data size fetched per partition.
- `max.poll.records`: Maximum number of records fetched per poll.
- `session.timeout.ms` and `heartbeat.interval.ms`: Control consumer liveness in the group.

[source,properties,role="copypaste"]
----
mp.messaging.incoming.meu-topico.connector=smallrye-kafka
mp.messaging.incoming.meu-topico.topic=meu-topico
mp.messaging.incoming.meu-topico.value.deserializer=org.apache.kafka.common.serialization.StringDeserializer
mp.messaging.incoming.meu-topico.group.id=grupo-consumidor-1

mp.messaging.incoming.meu-topico.auto.offset.reset=earliest
mp.messaging.incoming.meu-topico.enable.auto.commit=false
mp.messaging.incoming.meu-topico.fetch.min.bytes=50000
mp.messaging.incoming.meu-topico.fetch.max.wait.ms=500
mp.messaging.incoming.meu-topico.max.partition.fetch.bytes=1048576
mp.messaging.incoming.meu-topico.max.poll.records=100
mp.messaging.incoming.meu-topico.session.timeout.ms=15000
mp.messaging.incoming.meu-topico.heartbeat.interval.ms=5000
----
...
This configuration provides a good balance between performance, reliability, and efficiency for most Quarkus Kafka applications. These parameters should be further tuned based on your workload and latency requirements.

=== Hands-On Instructions

In this section, we will observe producer and consumer behavior in action.

. Print messages when producing and consuming events in the Quarkus application.
. Set `auto.offset.reset=earliest` and `group.id=${quarkus.uuid}` in the `application.properties` for the consumer.
. Deploy the application on OpenShift.
. Once deployed, scale the application to **two replicas**.
. Watch the logs in each replica to observe event consumption.
. Now, update the `group.id` parameter to `lab` and keep `auto.offset.reset=earliest`.
. Redeploy the application and check the logs again.

If everything is working correctly, you should see **parallel consumers** processing events independently across replicas.


Want more information with more examples? https://quarkus.io/guides/kafka
